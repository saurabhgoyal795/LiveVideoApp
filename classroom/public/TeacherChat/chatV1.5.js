 ...
 var secondaryApp = firebase.initializeApp(firebaseConfig, 'helloenglishfirestore');
...
var chatIos = getUrlVars()["chatIos"];
 ...
 function getUpdateListener() {
   ...
   .onSnapshot(function (doc) {
	 ...
	 var wn2 = document.getElementById('iframe2').contentWindow;
     ...
     if (url.trim() != lessonUrl.trim()) {
       ...
       if (autoTimer == true || autoTimer == "true") {
         $("#iframe").attr('src', lessonUrl.trim()+"&slide="+slideNumber+"&sessionId="+id+"&timerOn=true");
        } else {
         $("#iframe").attr('src', lessonUrl.trim()+"&slide="+slideNumber+"&sessionId="+id); 
        }
		if(isAdmin == true ||  isAdmin == "true"){
			$("#iframe2").attr('src', lessonUrl.trim()+"&slide="+(slideNumber+1)+"&sessionId="+id+"&isAdminPreview=true"); 
		}
     } else {
       if (slideNumber != slideNo){
         slideNo = slideNumber;
         wn.postMessage('slide='+slideNumber, "*");  
		 wn2.postMessage('slide='+(slideNumber+1), "*");  
       } else {
         if(showAnswer){
...
function getUrlParam(parameter, defaultvalue){
 ...
 function loadSlideNumber(number) {
   ...
   var wn2 = document.getElementById('iframe2').contentWindow;
   wn2.postMessage('slide='+(number+1), "*");  
 }
	...
	.collection("webinar").doc(id).collection("userData")
				.onSnapshot(function (querySnapshot) {
					...
					for(var i=0;i<studentData.length;i++){
						...
						if(studentData[i].name != undefined){
							$("#chooseBadgeUserName").append('<option value="'+studentData[i].name.split(" ")[0]+'/'+studentData[i].userId+'">'+studentData[i].name.split(" ")[0]+'('+studentData[i].userId+')</option>');
						}
					}
				});	
