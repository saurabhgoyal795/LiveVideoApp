...
function loadQuestions(type) {
	$.ajax({
		url : './audio_test_json/Part'+type+'.json',
		dataType: 'json',
		success : function(result) {
			...
			dataArray = result;
			var answers = []
			for (var i = 0; i < result.day0.length; i++) {
				// console.log('result type',result.Type)
				var totalTime = parseInt(result.day0[i].Length);
				...
				if (result.Type==1)
					slideHTML = slideHTML.replace('||questionText||',result.day0[i].Question_text)
				else {
					slideHTML = slideHTML.replace('||questionText||','')
					audioTest = true;
				}
				$('#outerSlides').append(slideHTML);
				answers.push(result.day0[i].Answer);
				// console.log('done')
			}
			$('.questionCounterText').html('1 of '+result.day0.length);
			var audioCount = 0;
			for(var j=0;j<dataArray.day0.length;j++){
				audioCount++;
				loadAudio(audioCount);
			}
			showSectionInstruction(0);
			Reveal.navigateNext();
			// initAudio();
		}
	});
...
function loadAudio(number){
	if (dataArray.Type == 2)
		$(".audioDiv").append('<audio id="audio'+number+'" class="audio audio'+number+'" text="" style="display:none;"><source class="source source'+number+'" src="./alex_voice/'+dataArray.day0[number-1].Filename+'" type="audio/mp3"></source></audio>');
	else
		$(".audioDiv").append('<audio id="audio'+number+'" class="audio audio'+number+'" text="" style="display:none;"><source class="source source'+number+'" src="'+number+'.mp3" type="audio/mp3"></source></audio>');
//	source.attr("src",'http://androidttsusingapp-env.elasticbeanstalk.com/file/'+window.parent.defaultLocale+'/'+processedText);
//	var x = $('.audio'+number)[0];
//	x.load();
...
function onUpdateSlide(slideNumber){
	...
	$("#bottomBarButton").attr("onclick","if($(this).attr('isDisable') == 'true'){return;}NextTestSlide();Reveal.navigateNext();");
...
$(document).keydown(function(evt) {
	...
	if (evt.keyCode == 32 && !recording_on) {
		...
		displayResult = true;
		if (tryAgainEnabled) {
			totalTime = parseInt(dataArray.day0[0].Length);
			drawTimer(0);
			var sec = parseInt(totalTime%60);
			var min = parseInt(totalTime/60);
			if(sec < 10){
		...
		recording_on = true;
		if (dataArray.day0[setCounter-1] != undefined){
			totalTime = parseInt(dataArray.day0[setCounter-1].Length);
			var sec = parseInt(totalTime%60);
			var min = parseInt(totalTime/60);
			if(sec < 10){
				sec = "0"+sec;
...
function stopRecording() {
	...
	if (!micTesting && TestType!='C' && totalTime == (parseInt(dataArray.day0[setCounter-1].Length))) {
		totalTime = parseInt(dataArray.day0[setCounter-1].Length);
		drawTimer(0);
		var sec = parseInt(totalTime%60);
		var min = parseInt(totalTime/60);
...
function NextTestSlide(){
	...
	if (useLanguageModel) {
		PS_config.pop()
		PS_config.push(['-lm','./'+dataArray.day0[currentRecordingId+1].Model])
	...
	if(currentQuestionNumber >= dataArray.day0.length){
		// loadTestData();
		allowKeydown = false;
		readyForKeydown = false;
	...
	$(".questionCounterText").html("Part "+TestType+": "+currentQuestionNumber+"/"+dataArray.day0.length);
	if(audioTimer != null){
		clearInterval(audioTimer);
	}
	if (!audioTest) {
...
function startAudioTest(){
	...
	if (!useGoogleSpeech)
		...
		currentQuestionNumber++;
		setCounter++;
		$(".questionCounterText").text(setCounter+" of "+totalTestQuestion);
...
function showExitInstructions(index){
	...
	$.ajax({
		url : 'https://helloenglish.com/saveUserFinalTestData.action?'+
					'email='+email+
					'&section='+TestType+
					'&numberOfQuestion='+dataArray.day0.length+
					'&date='+todate,
...
function showSectionInstruction(index){
	var html = '<div style="font-size: 32px;text-align: center;padding: 20px;">Section '+TestType+'</div>'+
	'<ul style="font-size: 16px;padding: 20px;">'+dataArray.Long_Instruction+'</ul>';
	var buttonHtml='<input onclick=\'startAudioTest();\''+
	'class="greenButton appStringValue" type="button" value="OK"'+
...
function startButton() {
	...
	if(recognizing){
		...
		recognizing = false;
    // recognition.stop();
    console.log(TestType, dataArray.day0[currentRecordingId], wavblob, URL.createObjectURL(wavblob));
    if (micTesting)
    	micTestUpload(wavblob);
    else
    	getPSScores(TestType, dataArray.day0[currentRecordingId], wavblob);
  }else{
  	recognizing = true;
  	isMicEnabled = false;
